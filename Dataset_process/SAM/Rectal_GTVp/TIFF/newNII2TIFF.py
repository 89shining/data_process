...
# ima（numpy数组）-> np.float32（32位浮点数TIFF）灰度图
def SaveMatTiff(ima, img_path, window_center=40, window_width=350):
    img = ima.astype(np.float32)
    img = (img - (window_center - window_width / 2)) / window_width
    img = np.clip(img, 0, 1)
    img = (img * 255).astype(np.uint8)
    Image.fromarray(img).save(img_path)        # 保存图像到指定路径
