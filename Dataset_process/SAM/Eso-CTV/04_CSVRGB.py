"""
对处理后的rgb_image.png和mask.nii生成新的CSV
"""
...
# 根目录路径
root_dir = r"C:\Users\dell\Desktop\Eso-CTV\20251217\dataset\test"  # rgb_images train/test根目录
...
for patient_id in sorted(os.listdir(pseudo_dir), key=lambda x: int(x.lstrip("p_"))):
    ...
    for file in sorted(os.listdir(patient_folder), key=lambda x: int(os.path.splitext(x)[0])):
        ...
        pseudo_rel = f"/rgb_images/{patient_id}/{file}"
...
csv_path = os.path.join(root_dir, "test_rgb.csv")
