...
def SaveMatPNG(mat, img_path):
    mat = np.clip(mat, 0, 255).astype(np.uint8)  # 确保是 0-255 范围，转换为 uint8
    im = Image.fromarray(mat, mode='L')  # L = 灰度模式
...
if __name__ == "__main__":
    ...
    srcdir = 'C:/Users/dell/Desktop/test46/test'      # srcdir下一级子目录是患者文件
    imadir = 'C:/Users/dell/Desktop/test46/image'    # 自己先创建images保存目录,不能在srcdir目录中，保存后子目录是患者文件夹
    maskdir = 'C:/Users/dell/Desktop/test46/mask'    # masks同上
    ...
    for pa in palist:      # 获取palist每一个元素pa
        ...
        for ni in range(0, nslice):    # 遍历每个图像的切片数
            ...
            tmpslice = np.clip(tmpslice, 0, 1) * 255  # 归一化到 [0, 255]
            tmpfile = dstdir + '/' + str(ni) + '.png'  # 构建目标文件的路径，以.tiff后缀保存
            SaveMatPNG(tmpslice, tmpfile)

        maskfile = padir + '/mask.nii.gz'   # 拼接出掩膜文件 CTV.nii.gz 的路径
        ...
        for ni in range(0, nslice):      # 遍历掩膜图像每个切片
            ...
            SaveMatPNG(tmpslice, tmpfile)     # 调用SaveMatPNG函数，将当前切片数据tmpslice保存为PNG(unit8）
