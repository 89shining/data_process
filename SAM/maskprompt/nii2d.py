...
提取2D切片
nii.gz -> nii
生成CSV文件：image.nii   mask.nii  prompt.nii  prompt_available(0,1)
"""
import csv
...
def GenerateNIICSV(rootdir, ctdir, maskdir, promoptdir, trainfile):
    ...
    for pa in palist:
        ...
        for fi in filist:
            ...
            if np.max(tmask) > 0:
                ...
                traininfo.append(promptfile.replace(rootdir, ''))
                traininfo.append(prompt_available)
                csv_qingxi.writerow(traininfo)   # 写入CSV文件

    fqingxi.close()

if __name__ == "__main__":
    srcdir = 'C:/Users/WS/Desktop/MRI_mask/datanii'  # nii.gz主目录
    rootdir = 'C:/Users/WS/Desktop/MRI_mask/dataset'   # dataset主目录
    csv_path = 'C:/Users/WS/Desktop/MRI_mask/dataset/train_nii.csv'  # 保存csv地址
    imadir = 'C:/Users/WS/Desktop/MRI_mask/dataset/images'   # image.nii保存地址
    maskdir = 'C:/Users/WS/Desktop/MRI_mask/dataset/masks'
    promptdir = 'C:/Users/WS/Desktop/MRI_mask/dataset/prompts'
    ...
    for pa in palist:
        ...
        maskfile = os.path.join(padir, 'GTVp.nii.gz')
        promptfile = os.path.join(padir, 'prompt.nii.gz')
        ct_img = sitk.ReadImage(ctfile)
        mask_img = sitk.ReadImage(maskfile)
        prompt_img = sitk.ReadImage(promptfile)

        ct_data = sitk.GetArrayFromImage(ct_img)
        mask_data = sitk.GetArrayFromImage(mask_img)
        prompt_data = sitk.GetArrayFromImage(prompt_img)
        # assert ct_data.shape == mask_data.shape == prompt_data.shape, f"Shape mismatch in {pa}"

        # 创建目标子目录
        ct_dstdir = os.path.join(imadir, pa)
        mask_dstdir = os.path.join(maskdir, pa)
        prompt_dstdir = os.path.join(promptdir, pa)
        os.makedirs(ct_dstdir, exist_ok=True)
        os.makedirs(mask_dstdir, exist_ok=True)
        os.makedirs(prompt_dstdir, exist_ok=True)
        ...
        for ni in range(mask_data.shape[0]):
            ...
            # 保存 prompt
            prompt_save_path = os.path.join(prompt_dstdir, f"{ni}.nii")
            SaveMatNII(prompt_slice, prompt_img, prompt_save_path)
    # 生成 CSV（只记录非空掩膜）
    GenerateNIICSV(rootdir, imadir, maskdir, promptdir, csv_path)
