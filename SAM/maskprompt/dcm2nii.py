...
将dicom 2D切片转为nii.gz
change name
提取image gt mask_prompt(MR-CT).nii.gz
"""
import csv
...
def GetFilesinFolder(pa_path):
    ...
    return ct_files, prompt_rs, gt_rs
...
if __name__ == "__main__":
    root_dir = "C:/Users/WS/Desktop/GTVp_MRI/rawdata"
    output_dir = "C:/Users/WS/Desktop/GTVp_MRI/datanii"
    os.makedirs(output_dir, exist_ok=True)
    csv_path = "C:/Users/WS/Desktop/GTVp_MRI/datanii/patient_id.csv"
    ...
    for pa in palist:
        print(pa)
        ct_files, prompt_rs, gt_rs = GetFilesinFolder(pa)
        if not (prompt_rs and gt_rs):
            print(f"跳过：{pa}，未找到符合命名规则的RS文件")
            continue
        ...
        gt_mask_path = os.path.join(sub_path, "GTVp.nii.gz")
        prompt_mask_path = os.path.join(sub_path, "prompt.nii.gz")
        ...
        try:
            extract_mask(pa_path, gt_rs, gt_mask_path, "GTVp", ct_files_sorted)
            extract_mask(pa_path, prompt_rs, prompt_mask_path, "MR-Reg GTVp", ct_files_sorted)
        except Exception as e:
            print(f"跳过：{pa}，提取 mask 时出错：{e}")
            continue
